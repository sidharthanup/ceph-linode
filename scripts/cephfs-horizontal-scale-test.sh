...
function do_test {
  run ans -m shell -a "/root/horizontal_scale_file_create.sh $1" "$1"
}
...
function main {
...

  {
    ...
    run ans --module-name=copy --args="src=misc/horizontal_scale_file_create.sh dest=/root/ owner=root group=root mode=755" clients
